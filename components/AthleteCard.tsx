...
import type { AthleteCardProps } from "@/types/ui";
        ...
        <div className="relative z-10 p-6">
          ...
          <div
            className={`grid overflow-hidden transition-all duration-[1400ms] ease-out ${
              selected
                ? "mt-8 max-h-[800px] opacity-100"
                : "max-h-0 opacity-0"
            }`}
          >
            <div className="grid gap-4 border-t border-white/[0.06] pt-6">
              {telemetry.map((item) => (
                <TelemetryBlock
                  key={item.label}
                  label={item.label}
    ...
    >
      <div className="flex items-center justify-between gap-6">
        <div className="min-w-0">
          ...
          <p className={`mt-3 text-xl font-semibold leading-none ${tone}`}>
            {value}
          </p>
        </div>

        <div className="w-28 flex-shrink-0 opacity-70">
          <svg
            viewBox="0 0 120 28"
